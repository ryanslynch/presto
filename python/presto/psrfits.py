...
def unpack_1bit(data: np.ndarray) -> np.ndarray:
    ...
    ----------
    data : numpy.ndarray
        An array of bits packed into an array of bytes.

    Returns
    -------
    numpy.ndarray
        The unpacked array, eight times the size of the input data.
    """
...
def unpack_2bit(data: np.ndarray) -> np.ndarray:
    ...
    ----------
    data : numpy.ndarray
        An array of unsigned 2-bit ints packed into an array of bytes.

    Returns
    -------
    numpy.ndarray
        The unpacked array, four times the size of the input data.
    """
...
def unpack_4bit(data: np.ndarray) -> np.ndarray:
    ...
    ----------
    data : numpy.ndarray
        An array of unsigned 4-bit ints packed into an array of bytes.

    Returns
    -------
    numpy.ndarray
        The unpacked array, twice the size of the input data.
    """
...
class PsrfitsFile(object):
    """
    A search-mode PSRFITS file, providing access to its spectra.

    Parameters
    ----------
    psrfitsfn : str
        The path to the PSRFITS file.
    ...
    fits : astropy.io.fits.HDUList
        The open FITS file.
    specinfo : SpectraInfo
        The parsed spectra information for the file.
    ...
    poln_order : str
        The polarization ordering (e.g. "AABBCRCI").
    nsamp_per_subint : int
        The number of spectra per subint.
    nsubints : int
        The number of subints.
    freqs, frequencies : numpy.ndarray
        The channel frequencies (aliases of one another).
    tsamp : float
        The sample time in seconds.
    nspec : int
        The total number of spectra in the file.
    zero_off : float
        The ZERO_OFF value applied to the data.
    """
    ...
    def read_subint(
        ...
        apply_zero_off: bool = True,
        total_intensity: bool = True,
    ) -> np.ndarray:
        """
        Read a PSRFITS subint, applying scales, weights, and offsets.

        Parameters
        ----------
        isub : int
            The index of the subint (the first subint is 0).
        apply_weights : bool, optional
            If True, apply weights (default True).
        apply_scales : bool, optional
            If True, apply scales (default True).
        apply_offsets : bool, optional
            If True, apply offsets (default True).
        apply_zero_off : bool, optional
            If True, apply ZERO_OFF (default True).
        total_intensity : bool, optional
            If True, and the data have 2+ polarizations, average them to
            total intensity (default True).

        Returns
        -------
        numpy.ndarray
            The subint data with scales, weights, and offsets applied, as
            float32 with shape ``(nsamps, [npoln,] nchan)``.
        """
        ...
        if self.nbits < 8:  # Unpack the bytes data
            if (shp[0] != self.nsamp_per_subint) and (
                ...
                sdata = sdata.reshape(
                    self.nsamp_per_subint, self.nchan * self.npoln * self.nbits // 8
                )
            if self.nbits == 4:
        ...
        if (
            total_intensity
            and len(shp) == 3
            and shp[1] == self.npoln
            and (self.poln_order == "AABBCRCI" or self.poln_order == "AABB")
        ):
    ...
    def get_weights(self, isub: int) -> np.ndarray:
        """
        Return the weights for a particular subint.
        ...
        -------
        numpy.ndarray
            The subint weights (one value per channel).
        """
        return self.fits["SUBINT"].data[isub]["DAT_WTS"]

    def get_scales(self, isub: int) -> np.ndarray:
        """
        Return the scales for a particular subint.
        ...
        -------
        numpy.ndarray
            The subint scales (one value per channel * npoln).
        """
        return self.fits["SUBINT"].data[isub]["DAT_SCL"]

    def get_offsets(self, isub: int) -> np.ndarray:
        """
        Return the offsets for a particular subint.
        ...
        -------
        numpy.ndarray
            The subint offsets (one value per channel * npoln).
        """
        return self.fits["SUBINT"].data[isub]["DAT_OFFS"]

    def get_spectra(self, startsamp: int, N: int) -> spectra.Spectra:
        """
        Return a block of spectra from the PSRFITS file.
        ...
        return spectra.Spectra(
            freqs, self.tsamp, data, starttime=self.tsamp * startsamp, dm=0
        )


class SpectraInfo(object):
    """
    Collected metadata for one or more search-mode PSRFITS files.

    Parses the primary and SUBINT headers of each file (which must all
    belong to the same contiguous observation) and computes derived
    quantities such as the total number of spectra, bandwidth, and the
    padding needed between files. Emulates the behavior of PRESTO's
    ``psrfits.c``.

    Parameters
    ----------
    filenames : list of str
        The PSRFITS files, in time order.
    """
        ...
        for ii, fn in enumerate(filenames):
            ...
            if ii == 0:
                ...
                if telescope != self.telescope:
                    warnings.warn(
                        "'TELESCOP' values don't match for files 0 and %d!" % ii
                    )
            ...
            self.start_MJD[ii] = (
                primary["STT_IMJD"]
                + (primary["STT_SMJD"] + primary["STT_OFFS"]) / pc.SECPERDAY
            )
            ...
            if envval is not None:
                ...
                if (ival > -1) and (ival < self.num_polns):
                    print(
                        "Using polarisation %d (from 0-%d) from PSRFITS_POLN."
            ...
            if "DAT_FREQ" not in subint_hdu.columns.names:
                ...
                if ii == 0:
                    ...
                    if np.any(np.abs(ftmp - self.df) > 1e-7):
    ...
    def __str__(self) -> str:
        """Format the spectra info into an easy-to-read string."""
        ...
        result.append("         Polarisation order = %s" % self.poln_order)
...
def DATEOBS_to_MJD(dateobs: str) -> tuple[float, float]:
    """
    Convert a DATE-OBS string from a PSRFITS primary HDU to an MJD.

    Parameters
    ----------
    dateobs : str
        The DATE-OBS card value, ``YYYY-MM-DDThh:mm:ss[.sss]``.

    Returns
    -------
    mjd_day : float
        The integer part of the MJD (the day).
    mjd_fracday : float
        The fractional part of the MJD (the time of day).
    """
...
def is_PSRFITS(filename: str) -> bool:
    """
    Return whether a file appears to be search-mode PSRFITS.
    ...
    -------
    bool
        True if the file appears to be PSRFITS in SEARCH mode, False
        otherwise.
    """
...
def debug_mode(mode: bool | None = None) -> bool | None:
    """
    Get or set the module's debugging mode.

    Parameters
    ----------
    mode : bool, optional
        If None (default), return the current debug mode. Otherwise, set
        the debug mode to ``bool(mode)``.

    Returns
    -------
    bool or None
        The current debug mode if `mode` is None, otherwise None.
    """
    global debug
    if mode is None:
        return debug
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "files", metavar="FILE", nargs="+", type=str, help="PSRFITS files."
    )
    parser.add_argument(
        "-o",
        type=str,
