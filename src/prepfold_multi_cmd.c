...
/*****
  This is a modified version of the *prepfold* parser.  It populates the
  SAME struct s_Cmdline layout that prepfold uses -- prepfold_multi_cmd.h
  is prepfold_cmd.h with a single `candfile` field appended AFTER
  full_cmd_line, so every field the shared prepfold_pipeline.c code
  touches keeps prepfold's offsets.  prepfold_multi therefore exposes only
  the options that make sense when folding many candidates from a file;
  the single-candidate spin/source/binary/event options remain in the
  struct (defaulted, never set) so the shared pipeline compiles and links
  unchanged.
*****/
...
void usage(void)
{
    fputs("   [-ncpus ncpus] [-o outfile] [-filterbank] [-psrfits] [-noweights] [-noscales] [-nooffsets] [-topo] [-invert] [-zerodm] [-samples] [-normalize] [-if ifs] [-clip clip] [-noclip] [-noxwin] [-runavg] [-fine] [-coarse] [-slow] [-searchpdd] [-searchfdd] [-nosearch] [-nopsearch] [-nopdsearch] [-nodmsearch] [-scaleparts] [-allgrey] [-fixchi] [-justprofs] [-n proflen] [-nsub nsub] [-npart npart] [-pstep pstep] [-pdstep pdstep] [-dmstep dmstep] [-npfact npfact] [-ndmfact ndmfact] [-phs phs] [-start startT] [-end endT] [-mask maskfile] [-ignorechan ignorechanstr] [-candfile candfile] [--] infile ...\n", stderr);
    fputs("      Folds many pulsar candidates (each with its own period/frequency, derivatives, and DM) in a single pass over the raw data.\n", stderr);
    fputs("         -ncpus: Number of OpenMP threads (CPUs) used to fold candidates (default: min(host CPU count, number of candidates))\n", stderr);
    ...
    fputs("             -n: Profile bins applied to ALL candidates (default: chosen automatically per candidate from its period)\n", stderr);
    fputs("                 1 int value\n", stderr);
    ...
    fputs("                 1 char* value\n", stderr);
    fputs("      -candfile: Text file listing candidates to fold (one per row; '#' header names the columns -- see prepfold_multi(1))\n", stderr);
    fputs("                 1 char* value\n", stderr);
    fputs("         infile: Input data file name.  Must be raw radio data (PSRFITS or SIGPROC filterbank) or PRESTO subbands (*.subNN); already-dedispersed .dat time series and event files are not supported (no per-candidate DM).\n", stderr);
