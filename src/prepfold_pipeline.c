...
/* Autogenerated candidate name, matching prepfold's historical format.  Shared */
/* by resolve_output_names and prepfold_multi so the name format is one place.   */
char *make_autogen_candnm(int is_period, double x0)
{
    char *nm = (char *) calloc(32, sizeof(char));
    if (is_period)
        sprintf(nm, "%.2fms_Cand", x0 * 1000.0);
    else
        sprintf(nm, "%.2fHz_Cand", x0);
    return nm;
}

/* Build the .pfd / .pfd.ps / pgplot-device names from rootnm and search->candnm */
/* (which must already be set).  Shared by resolve_output_names and prepfold_multi. */
void build_output_filenames(foldcand *fc, char *rootnm)
{
    prepfoldinfo *search = &fc->search;
    int slen = strlen(rootnm) + strlen(search->candnm) + 6;

    fc->outfilenm = (char *) calloc(slen, sizeof(char));
    sprintf(fc->outfilenm, "%s_%s.pfd", rootnm, search->candnm);
    fc->plotfilenm = (char *) calloc(slen + 3, sizeof(char));
    sprintf(fc->plotfilenm, "%s_%s.pfd.ps", rootnm, search->candnm);
    search->pgdev = (char *) calloc(slen + 7, sizeof(char));
    sprintf(search->pgdev, "%s/CPS", fc->plotfilenm);
}

/* The default profile-length rule (prepfold's): explicit -n wins, else one    */
/* sample-bin per folded period capped at 64 (rounded up to a power of 2 under */
/* -timing).  Shared by resolve_fold_params and prepfold_multi.                */
...
/* Subtract the dispersion delay to infinite frequency from a barycentric epoch */
/* (no-op when dm <= 0).  Shared by compute_obs_timing and prepfold_multi.       */
double bary_epoch_to_infinite_freq(infodata *idata, double bepoch, double dm)
{
    if (dm > 0.0) {
        double barydispdt = delay_from_dm(dm, idata->freq +
                                          (idata->num_chan - 1) * idata->chan_wid);
        bepoch -= (barydispdt / SECPERDAY);
    }
    return bepoch;
}
...
void resolve_output_names(Cmdline *cmd, foldcand *fc, char *rootnm)
{
    ...
    if (cmd->psrnameP) {
        ...
        if (cmd->pP)
            search->candnm = make_autogen_candnm(1, cmd->p);
        else if (cmd->fP)
            search->candnm = make_autogen_candnm(0, cmd->f);
        else {
    ...
    if (RAWDATA || insubs) {
        ...
        if (idata->mjd_i) {
            ...
            if (!cmd->polycofileP && !cmd->timingP && !cmd->topoP && !cmd->parnameP) {
                ...
                search->bepoch =
                    bary_epoch_to_infinite_freq(idata, search->bepoch, fc->dm);
            }
        }
        worklen = ptsperrec;
...
{
    double *barytimes, *topotimes, *voverc, avgvoverc = 0.0;
    int numbarypts;
    long ii;
...
/* Single-candidate convenience wrapper (prepfold's path): build the shared     */
/* table then apply this candidate's correction.  Identical behaviour/output to */
/* the pre-split function, so the prepfold fold_timeseries call is unchanged.    */
void compute_bary_corrections(Cmdline *cmd, foldcand *fc, double T,
                              ...
                              double **barytimes_out, double **topotimes_out)
{
    double avgvoverc = 0.0;
    (void) cmd;

    compute_bary_table(fc->search.tepoch, T, rastring, decstring, obs, ephem,
                       numbarypts_out, barytimes_out, topotimes_out, &avgvoverc);
    fc->search.avgvoverc = avgvoverc;
    apply_bary_fold_correction(fc, *numbarypts_out, *barytimes_out, *topotimes_out);
}
...
/* Fold one cleaned+dedispersed raw block into a single candidate's profiles.  */
/* `data` holds fc->nsub subbands, each `worklen` points (subband-major, as    */
/* produced by dedisp_subbands()/read_subbands()).  This is the per-candidate  */
/* inner step of the fold: it is shared verbatim by fold_timeseries() (single  */
/* candidate) and by prepfold_multi's read-once/dedisperse-many loop, so the   */
/* fold math exists in exactly one place.  foldf/foldfd/foldfdd and fold_time0 */
/* are passed in (not read from fc) because they vary per block under polycos. */
...
#pragma omp parallel for default(shared)
    ...
    for (kk = 0; kk < fc->nsub; kk++) {
        ...
        fold(data + kk * worklen, numread, search->dt,
             fold_time0,
             search->rawfolds + (partnum * fc->nsub + kk) * search->proflen,
             search->proflen, fc->phs, buffers + kk * search->proflen,
             phasesadded + kk, foldf, foldfd, foldfdd, fc->flags, fc->Ep, fc->tp,
             fc->numdelays, NULL, &(search->stats[partnum * fc->nsub + kk]),
             !cmd->samplesP);
    }
}
                     ...
                     double **barytimes_out, double **topotimes_out)
{
    ...
    for (ii = 0; ii < fc->npart; ii++) {
        ...
        for (jj = 0; jj < reads_per_part; jj++) {
            ...
            fold_subband_block(cmd, fc, data, worklen, numread, ii, fold_time0,
                               foldf, foldfd, foldfdd, buffers, phasesadded);
