...
### **What's the `-candfile` format for `prepfold_multi`?**

`prepfold_multi` folds many pulsar candidates (each with its own period or
frequency, spin derivatives, and DM) in a single pass over the raw data,
rather than re-reading the data once per candidate like ordinary `prepfold`.
The full details are in `man prepfold_multi` (since PRESTO's man pages aren't
always the first place people look, here's a copy of the important part, the
`-candfile` format):

The file given to `-candfile` is a whitespace/tab-separated text table, one
candidate per row.

The first non-blank line is the *header* and must begin with `#`, followed by
one or more column keys separated by whitespace. Column keys are
case-insensitive. Recognized keys are:

- **name**: Optional. The candidate's name, used to build its output
  filenames (`.pfd`, `.bestprof`, etc.), exactly as the corresponding
  `prepfold` options would. If omitted, an output name is autogenerated from
  the candidate's period or frequency, exactly as `prepfold` autogenerates
  names for candidates given via `-p`/`-f`. If two candidates would resolve to
  the same name (two unnamed candidates with the same period, or two rows
  with the same explicit `name`), `prepfold_multi` disambiguates the later
  one with a `_2`, `_3`, ... suffix and prints a warning, rather than letting
  one candidate's output silently clobber another's.
- **DM**: Required. The candidate's dispersion measure (cm^-3 pc). Must be
  >= 0.
- **p**: The candidate's folding period (s). Must be > 0. Selects *period*
  mode for the whole file (see below).
- **f**: The candidate's folding frequency (Hz). Must be > 0. Selects
  *frequency* mode for the whole file (see below).
- **pd**: Optional. The period derivative (s/s). Only valid in period mode.
- **pdd**: Optional. The period 2nd derivative (s/s^2). Only valid in period
  mode.
- **fd**: Optional. The frequency derivative (Hz/s). Only valid in frequency
  mode.
- **fdd**: Optional. The frequency 2nd derivative (Hz/s^2). Only valid in
  frequency mode.

The header must contain exactly one of `p` or `f`, never both: this choice is
file-wide and fixes whether every candidate in the file is specified in
period mode (`p`, optionally with `pd`/`pdd`) or frequency mode (`f`,
optionally with `fd`/`fdd`). Mixing modes within one file is rejected: a
header may not combine `p` with `fd` or `fdd`, nor combine `f` with `pd` or
`pdd`. The header may declare at most 8 columns.

In the body of the file, blank lines and lines beginning with `#` are ignored
as comments. Every other line is a data row and must supply exactly as many
whitespace-separated fields as the header declared, in the same column order
as the header. Numeric fields (DM, p/f, and any derivatives) are parsed like
C's `strtod(3)`, so ordinary decimal and scientific notation (e.g.
`8.66430621957513e-3`) are both accepted. DM must be >= 0 and the period (or
frequency) must be > 0 on every row, or `prepfold_multi` reports the
offending line number and exits before any folding begins.

Example (period mode, three candidates at nearby trial DMs, sharing one spin
solution -- see `tests/prepfold/multi_cands.txt`):

    # name      DM      p                    pd
    candDM238   238.30  8.66430621957513e-3  -5.01154755640048e-11
    candDM240   240.30  8.66430621957513e-3  -5.01154755640048e-11
    candDM242   242.30  8.66430621957513e-3  -5.01154755640048e-11
